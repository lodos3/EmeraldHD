...
namespace MagicLightProbes
{
    ...
    public class MagicLightProbes : MonoBehaviour
    {
        ...
        private ComputeShader FindShader(string name, string workPatch)
        {
            ...
            {
                string[] finalPath = workPatch.Split(new string[] {"Assets"}, StringSplitOptions.None);

                resultShader = AssetDatabase.LoadAssetAtPath<ComputeShader>("Assets/" + finalPath[1].Substring(1) + COMPUTE_SHADERS_FOLDER + name + ".compute");
            }
            else
            {
            ...
            if (!calculatingVolume)
            {
                ...
                if (subVolumesDivided.Count == 0)
                {
                    ...
                    if (update)
                    {
                        if (useDynamicDensity)
                        {
                            xPointsCount = Mathf.RoundToInt(probesVolume.transform.localScale.x / cornersDetectionThresholdMax);
                            yPointsCount = Mathf.RoundToInt(probesVolume.transform.localScale.y / cornersDetectionThresholdMax);
                            zPointsCount = Mathf.RoundToInt(probesVolume.transform.localScale.z / cornersDetectionThresholdMax);
                        }
                        else
                        {
                ...
                else
                {
                    if (useDynamicDensity && redivideParts)
                    {
                        ...
                        int currenrVolumesCount = subVolumesDivided.Count;

                        for (int i = 0; i < currenrVolumesCount; i++)
                        {
            ...
            switch (boundsDisplayMode)
            {
                case BoundsDisplayMode.Always:
                    if (Selection.activeGameObject != null)
                    {
                        if (Selection.activeGameObject == probesVolume)
                        {
                            probesVolume.GetComponent<MLPVolume>().showGizmo = false;

                            if (subVolumesDivided.Count > 0)
                            {
                                probesVolume.GetComponent<MLPVolume>().showGizmoSelected = false;

                                for (int i = 0; i < subVolumesDivided.Count; i++)
                                {
                                    subVolumesDivided[i].GetComponent<MLPVolume>().showGizmo = false;
                                    subVolumesDivided[i].GetComponent<MLPVolume>().showGizmoSelected = true;
                                }
                            }
                            else
                            {
                                probesVolume.GetComponent<MLPVolume>().showGizmoSelected = true;
                            }
                        }
                        else if (Selection.activeGameObject == gameObject)
                        {
                            ...
                            if (subVolumesDivided.Count > 0)
                            {
                                ...
                                {
                                    subVolumesDivided[i].GetComponent<MLPVolume>().showGizmo = false;
                                    subVolumesDivided[i].GetComponent<MLPVolume>().showGizmoSelected = false;
                                }
                            }
                        }
                        else
                        {
                            probesVolume.GetComponent<MLPVolume>().showGizmo = true;
                            //probesVolume.GetComponent<MLPVolume>().showGizmoSelected = true;
            ...
            if (!calculatingVolume)
            {
                ...
                foreach (var volume in allVolumes)
                {
                    if (volume != this)
                    {
                        ...
                        {
                            if (probesVolume.GetComponent<MeshRenderer>().bounds.Contains(bp))
                            {
                                volumeInside = true;
                                break;
                            }
        ...
        private List<GameObject> PrioritizeVolumeList(List<GameObject> volumes)
        {
            ...
            {
                if (volumes[i].transform.IsVisibleFrom(SceneView.lastActiveSceneView.camera))
                {
